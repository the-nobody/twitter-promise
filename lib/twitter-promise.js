   ...
   switch (_options.method) {
    ...
    return new Promise((resolve,reject) => {
      ...
      if (this._shouldBackoff(opts.path)) {
        reject({type: 'twitter-promise:backoff', err: {path: opts.path, backoff: this._backoff[opts.path]}});
      }
      
      // make request to twitter
      this._request(_options, (err, res, data) => {
        if (err) {
          reject({type:'twitter-promise:request', err});
        }
        ...
        this._checkBackoff(opts.path, res.headers)
          ...
          .catch(err => {
            reject({type: 'twitter-promose:checkapi', error: {err,res,data}});
          });
      });
  ...
  stream(track) {
    ...
    this.activeStream.on("response", response => {
      ...
      if(response.statusCode !== 200) {
        ...
        return _request.abort(); // abort reqeust
      }
        ...
        while ((index = buffer.indexOf("\r\n")) > -1) {
          ...
          if (json.length) {                      // if the json has length try to parse and emit a success/error event
            try {
              ...
              console.log("WE GOT A NON JSON: ", err, json);
            }
          }
        }
      })
