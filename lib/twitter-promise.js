...
class Twitter {
  ...
  // helper function to set back off status for an end point based on the api response from the headers.
    ...
    if (headers.status !== "200 OK") {
      return new Error("STATUS FAIL", path, headers.status);
    }
    
    return new Promise((resolve, reject) => {
      if (!api.remain) {
        this._backoff[path] = api.reset;
        reject({ msg: `${path} rate limit reached. Checkback at ${api.reset}`, api });
      }
      else {
        resolve({ msg: "success", api });
      }
    })
  }
  ...
  stream(track) {
    ...
    _request
      .on("response", response => {
        ...
        // response end
        .on("end", () => {
          this.events.emit("twitter:stream:end", response);
        });
      })
      // request error
      .on("error", err => {
